...
module.exports = async (req, res) => {
    ...
    try {
        // --- ETAPA 1: Abrir a tarefa e pegar os detalhes completos DIRETAMENTE do servidor oficial ---
        ...
        taskData.questions.forEach(question => {
            // Método 1: A resposta está diretamente no objeto da questão (ex: preencher lacunas)
            if (question.answer) {
            ...
            // Método 2: A resposta precisa ser construída a partir das opções (ex: múltipla escolha)
            } else if (question.options && Array.isArray(question.options)) {
                const answerForQuestion = {};
                let hasCorrectOption = false;
                question.options.forEach(option => {
                    answerForQuestion[option.id] = option.is_correct;
                    if(option.is_correct) hasCorrectOption = true;
                });
                if (hasCorrectOption) {
                    correctAnswers[question.id] = {
                        question_id: question.id,
        ...
        // --- ETAPA 4: Montar e enviar a tarefa para o servidor oficial ---
        const submitPayload = {
